...
`category` can be any non-empty label. The seeded categories have tailored icons in
`src/lib/blog-presentation.ts`; new categories automatically use a generic book icon, so adding a
post never requires a second content list. Set `featured: true` on at most one post to use it as the
large lead story. The content check reports a clear build error when metadata is missing, a slug is
invalid, more than one post is featured, or the ordered `sections` IDs do not match the post's `##`
headings.
